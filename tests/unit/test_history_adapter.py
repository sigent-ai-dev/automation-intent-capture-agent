...
async def test_save_calls_put_item(adapter, mock_history):
    with patch("voice_server.persistence.history_adapter.put_item", new_callable=AsyncMock) as mock_put:
...
async def test_load_returns_data(adapter):
    ...
    item = {
        ...
        "turns": {"L": [{"M": {"role": {"S": "user"}, "text": {"S": "hello"}, "timestamp": {"S": "2026-05-27T10:00:00+00:00"}}}]},
        "window_size": {"N": "10"},
        "updated_at": {"S": "2026-05-27T10:05:00+00:00"},
        "expires_at": {"N": str(now + 86400)},
    }
    with patch("voice_server.persistence.history_adapter.get_item", new_callable=AsyncMock) as mock_get:
